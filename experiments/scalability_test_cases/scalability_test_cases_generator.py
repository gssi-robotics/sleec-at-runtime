...
Usage: scalability_test_cases_generator.py --name NAME -r R -c C -n N
...
def generate_all(name: str, R: int, C: int, N: int, base_dir: str | None = None) -> dict:
    ...
    asm_path = asm_generator.generate_and_write(name, R, C, str(models_dir))
    testcases_path = test_cases_generator.generate_test_cases(name, R, C, N, str(tests_dir))
...
def main(argv=None):
    ...
    parser.add_argument("--name", required=True, help="base name for artifacts")
    ...
    res = generate_all(args.name, args.rules, args.clauses, args.num, args.out)
