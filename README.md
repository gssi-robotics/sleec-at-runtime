...
This is the replication package for the paper _Enforcing Ethics at Runtime in Autonomous Systems_ submitted to SEAMS2026.
...
|   |   run_tests.sh                            # Test running entrypoint
...
cp run_tests.sh ../enforcement_subsystem/ros2_ws/
...
- The `results` folder contains:
    - All the collected logs during the experiments: local execution of 250 test cases (`local-seams250`), robot execution of 250 and 500 test cases (`seams250`, `seams500`) and  _ping_ result logs (`ping.txt`).
...
- The `test_cases` folder contains a set of .json files for test cases of different sizes. The test cases sets in `seams250.json` and `seams500.json` are the one used in the paper.
...
scp experiments/run_tests.sh <user>@<host>:<folder_path>/ros2_ws
```
